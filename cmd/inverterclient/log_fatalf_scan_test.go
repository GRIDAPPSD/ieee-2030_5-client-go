...
// Regression scan: log.Fatalf call-site inventory in main.go.
//
// Every log.Fatalf call in cmd/inverterclient/main.go that fires on an HTTP
...
// optional. This scan reads cmd/inverterclient/main.go and asserts:
//
//   1. The two graceful-bypass call sites (Phase 1b time sync, Phase 3 DER
//      list GET) stay gone. The patterns that used to trigger Fatalf there
//      are unmistakable; if either resurfaces, this test fails.
//
//   2. The set of REMAINING `log.Fatalf(` call lines in main.go matches an
//      explicit allow-list of essential/by-design sites (boot path,
//      phase2b/2c/2c exit-code owners). Any new `log.Fatalf(` line outside
//      this allow-list fails the test and forces explicit audit: nobody
//      adds a new fatal call site without updating this scan.
//
...
// for graceful bypass, as Phase 1b / Phase 3 were), drop its entry here.
...
func TestLogFatalfCallSites(t *testing.T) {
	...
	for _, line := range callLines {
		...
		if !matched {
			t.Errorf("unexpected log.Fatalf call site in main.go: %s\nadd it to expectedFatalfSitePrefixes with a justification comment, or replace with graceful bypass", line)
		}
	}
	...
	// not return. Match on a distinctive substring of each.
	forbidden := []struct {
		needle string
		reason string
	}{
